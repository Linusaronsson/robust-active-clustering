...
class data_train:
    ...
    def get_acc_on_set(self, test_dataset):
        ...
        loader_te = DataLoader(test_dataset, shuffle=False, pin_memory=False, batch_size=batch_size)
        self.clf.eval()
        accFinal = 0.
    ...
    def train(self, gradient_weights=None):
        ...
        if self.args['isreset']:
            self.clf = self.clf.apply(weight_reset).to(device=self.device)

        if self.args['optimizer'] == 'sgd':
            optimizer = optim.SGD(self.clf.parameters(), lr = self.args['lr'], momentum=0.9, weight_decay=5e-4)
            #optimizer = optim.SGD(self.clf.parameters(), lr = self.args['lr'], momentum=0.9, weight_decay=0.001)
        ...
        loader_tr = DataLoader(self.training_dataset, batch_size=batch_size, shuffle=True, pin_memory=False)
        epoch = 1
        accCurrent = 0
        is_saturated = False
        acc_monitor = []
        ...
        while (accCurrent < self.args['max_accuracy']) and (epoch < n_epoch) and (not is_saturated): 
            ...
            if self.args['islogs']:
                train_logs.append({
                    ...
                    #"model_state_dict": copy.deepcopy(self.clf.state_dict()),
